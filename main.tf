...
resource "aws_instance" "hadoop_namenode" {
  ...
  provisioner "file" {
    ...
    destination = "/tmp/configure_node.sh"
  }


  provisioner "remote-exec" {
    inline = [
      "chmod +x /tmp/configure_node.sh",
      "sed -i -e 's/\r$//' /tmp/configure_node.sh",
  ...
  provisioner "remote-exec" {
    inline = [
      "ssh-keyscan -H ${aws_instance.hadoop_namenode.public_dns} >> ~/.ssh/known_hosts",
